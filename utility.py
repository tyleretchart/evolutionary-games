class Utility:
    ...
    def of(self, row_player, col_player):
        pairings = [
            self._all_a_all_a, self._all_a_all_b, self._all_a_tft,
            self._all_a_not_tft, self._all_b_all_a, self._all_b_all_b,
            self._all_b_tft, self._all_b_not_tft, self._tft_all_a,
            self._tft_all_b, self._tft_tft, self._tft_not_tft,
            self._not_tft_all_a, self._not_tft_all_b, self._not_tft_tft,
            self._not_tft_not_tft
        ]

        assert row_player in self.players
        assert col_player in self.players

        return pairings[self.players.index(row_player) * 4 +
                        self.players.index(col_player)]()
